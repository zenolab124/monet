fn main() {
  ...
  #[cfg(target_os = "macos")]
  {
    ...
    println!(
      "cargo:rustc-link-arg-bin=cc-space-routine-runner={}/libcc_space_tcc_check.a",
      out_dir
    );
    for arg in ["-framework", "CoreServices", "-framework", "ApplicationServices"] {
      println!("cargo:rustc-link-arg-bin=cc-space-routine-runner={}", arg);
    }
  }
  tauri_build::build()
}
