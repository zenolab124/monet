...
enum WidgetPalette {
    /// 分类槽位：蓝 → 青绿 → 淡紫 → 琥珀 → 玫红
    /// 前三槽冷色——大段不触发暖色前进感（chromostereopsis）
    static func categorical(_ scheme: ColorScheme) -> [Color] {
        scheme == .dark ? dark : light
    }

    private static let dark: [Color] = [
        Color(hex: 0x3987E5), Color(hex: 0x199E70), Color(hex: 0x9085E9),
        Color(hex: 0xC98500), Color(hex: 0xD55181),
    ]
    private static let light: [Color] = [
        Color(hex: 0x2A78D6), Color(hex: 0x16966C), Color(hex: 0x6153C6),
        Color(hex: 0xB37A00), Color(hex: 0xD55181),
    ]
}
