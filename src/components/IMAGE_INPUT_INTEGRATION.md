...
pub fn write_temp_image(base64_data: String, ext: String) -> Result<String, String> {
    ...
    let dir = std::env::temp_dir().join("monet-images");
...
- 每次发送前清空 `temp_dir()/monet-images/` 下超过 1 小时的文件(避免堆积)
...
B64=$(base64 -i /tmp/monet-test-real.png | tr -d '\n')
...
Claude **正确识别了图片内容**(测试图是本项目旧版 logo:青蓝 + 橙黄 + "8")。
✓ 方案 a 可行。
...
claude --print --session-id "$TEST_SID" \
    "请描述图片 @/tmp/monet-test-real.png 的颜色,一句话" \
...
```bash
claude --image /tmp/monet-test-real.png --print ...
