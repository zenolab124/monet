...
function skip() {
  emit('decide', 'deny', { message: '用户跳过了本次提问' })
}
...
<template>
  <div
    ...
    role="dialog"
    aria-label="Claude 在提问"
  >
    <!-- 头部 -->
    <div class="flex items-center gap-2 px-3 py-2 border-b border-border">
      <span class="i-carbon-help w-4 h-4 text-primary shrink-0" aria-hidden="true" />
      <span class="text-sm font-medium text-foreground">Claude 在提问</span>
      <span class="text-xs text-muted-foreground">选择后提交，答案将回传给 Claude</span>
    </div>
    ...
    <div class="px-3 py-2 max-h-96 overflow-y-auto space-y-3">
      <div v-for="(q, qi) in questions" :key="qi">
        <div class="flex items-center gap-1.5 flex-wrap text-xs">
          ...
          <span v-if="q.multiSelect" class="text-muted-foreground shrink-0">（可多选）</span>
        </div>
    ...
    <div class="flex items-center gap-2 px-3 py-2 border-t border-border">
      ...
      <span v-if="!allAnswered" class="text-xs text-muted-foreground">每个问题都需作答</span>
      <div class="flex-1" />
