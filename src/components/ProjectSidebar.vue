...
<template>
  <div class="h-full flex flex-col" @contextmenu.prevent>
    ...
    <!-- 加载态 -->
    <div v-if="loading" class="px-3 py-8 text-center text-default3 text-xs">
      正在加载项目...
    </div>
    ...
    <div v-else-if="projects.length === 0" class="px-3 py-8 text-center">
      <p class="text-default3 text-xs">未发现任何项目</p>
      <p class="text-default4 text-xs mt-1">请确认 ~/.claude/projects 目录存在</p>
    </div>
    ...
    <div v-else class="flex-1 overflow-y-auto min-h-0 overscroll-y-contain">
      <button
        ...
        :class="{ 'bg-active': isSelected(project.id) }"
        @click="toggleProject(project.id)"
      >
        <!-- 选中指示 -->
        <span
          class="w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 transition-colors"
          :class="isSelected(project.id) ? 'bg-blue-500' : 'bg-transparent group-hover:bg-default4'"
        />
        <div class="min-w-0 flex-1">
          <div class="text-sm text-default truncate" :title="project.display_path">
            {{ projectName(project.display_path) }}
          </div>
          <div class="text-xs text-default4 flex gap-2 mt-0.5">
    ...
    <!-- 底部统计 -->
    <div class="px-3 py-2 border-t border-divider text-xs text-default4 flex gap-3">
