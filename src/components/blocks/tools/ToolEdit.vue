...
<template>
  <div class="mt-2 rounded-md bg-background border border-border px-3 py-2 text-xs">
    <div class="flex items-center gap-1.5">
      ...
      <span v-if="replaceAll" class="ml-1 px-1.5 py-0.5 rounded border border-accent/50 text-accent text-2xs">全部替换</span>
    </div>
    <div v-if="oldString || newString" class="mt-2 space-y-1">
      <pre v-if="oldString" class="rounded bg-destructive/10 border border-destructive/20 px-2 py-1 text-destructive whitespace-pre-wrap break-all font-mono">- {{ oldString }}</pre>
      <pre v-if="newString" class="rounded bg-primary/10 border border-primary/20 px-2 py-1 text-primary whitespace-pre-wrap break-all font-mono">+ {{ newString }}</pre>
    </div>
  </div>
</template>
