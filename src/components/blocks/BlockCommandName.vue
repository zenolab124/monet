...
const props = defineProps<{
  block: { type: 'command-name' | 'command-args' | 'local-command-stdout'; text: string; [key: string]: unknown }
}>()
</script>

<template>
  <!-- command-name: /xxx -->
  <div v-if="props.block.type === 'command-name'" class="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
    <span class="i-carbon-terminal w-3 h-3 shrink-0" />
    <code class="bg-muted px-1 py-0.5 rounded text-xs">{{ props.block.text }}</code>
  </div>
  <!-- command-args: 参数 -->
