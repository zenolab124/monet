...
export type AppSection = 'workbench' | 'sessions' | 'search' | 'settings' | 'workshop' | 'automation'
...
function loadState(): UiState {
  try {
    ...
    if (raw) {
      ...
      const validSections: AppSection[] = ['workbench', 'sessions', 'search', 'settings', 'workshop', 'automation']
      return {
